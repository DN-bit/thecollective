...
async def ask_collective(q: str, judge_name: Optional[str] = None):
    ...
    try:
        ...
        # 1. Extract likely token symbol from query (1-6 char uppercase word)
        import re
        words = q.upper().split()
        # Common tokens + any short uppercase-looking word
        ...
        # 2. Search corpus for relevant entries
        corpus_rows = await conn.fetch("""
            ...
            GROUP BY c.event_id, c.description, c.domain, c.confidence, c.judged, c.judge_scores, c.created_at
            ORDER BY c.created_at DESC
            LIMIT 8
        """, f"%{q.split()[0]}%")

        # Also search by token if found
        token_rows = []
        if token_hint and token_hint != q.split()[0].upper():
        ...
        seen = set()
        all_rows = []
        for r in list(corpus_rows) + list(token_rows):
        ...
        # 5. Build context for Claude synthesis
        corpus_context = ""
        if all_rows:
...
Answer the question directly using this internal intelligence. If you don't have enough data, say so briefly and suggest what to look for."""
