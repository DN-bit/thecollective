    ...
    try:
        ...
        corpus_rows = await conn.fetch("""
            SELECT c.event_id, c.description, c.domain, c.confidence, c.judged,
                   ...
                   AVG(j.aggregate_score) as avg_score
            FROM corpus c
            ...
            LIMIT 8
        """, f"%{q.split()[0]}%")  # Search first meaningful word
        ...
        if token_hint:
            conv_rows = await conn.fetch("""
                SELECT j.judge_name, j.aggregate_score, j.bias_tag, j.notes
                FROM judgments j
            ...
            """, f"%{token_hint}%")
            if conv_rows:
                scores = [r['aggregate_score'] for r in conv_rows if r['aggregate_score']]
                if scores:
