...
async def ask_collective(q: str, judge_name: Optional[str] = None):
    """
    Natural language question answered using corpus, team conviction, and live X sentiment.
    q: free-form question e.g. "what's going on with pumpfun"
    judge_name: optional, personalizes the conviction context
    """
    try:
        conn = await get_db_conn()

        # 1. Extract likely token symbol from query
        import re
        words = q.upper().split()
        ...
        for w in words:
            ...
            if clean in known_tokens or (2 <= len(clean) <= 6 and clean.isalpha() and clean not in
               {"WHY","WHAT","WHEN","WHERE","HOW","THE","AND","FOR","IS","ARE","WAS","DOWN","UP","WITH","ABOUT","TODAY","GOING","ON","OFF"}):
                token_hint = clean
                break

        # Build search keywords — token first, then meaningful words from query
        stop_words = {"why","what","when","where","how","the","and","for","is","are","was",
                      "down","up","with","about","today","going","on","off","a","an","in","it",
                      "happening","doing","think","about","tell","me","latest","news"}
        search_keywords = []
        if token_hint:
        ...
        # 2. Search corpus — try token first, then keywords
        all_rows = []
        seen = set()

        async def corpus_search(term, limit=8):
            return await conn.fetch("""
                SELECT c.event_id, c.description, c.domain, c.confidence, c.judged,
                       c.judge_scores, c.created_at,
        ...
        # 3. Get team conviction for token if applicable
        conviction_summary = ""
        if token_hint:
            conv_rows = await conn.fetch("""
                SELECT j.judge_name, j.average_score, j.bias_tag, j.notes
                FROM judgments j
            ...
            if conv_rows:
                ...
                if scores:
                    ...
                    conviction_summary = f"Team conviction on {token_hint}: avg score {avg:.1f}/5 across {len(judges)} judge(s) ({', '.join(judges[:4])})"

        # 4. Get latest narrative history for token
        narrative_summary = ""
        if token_hint:
            nh = await conn.fetchrow("""
                SELECT sentiment_label, crowd_sentiment_score, sentiment_summary,
            ...
            """, token_hint)
            if nh:
                score = nh['crowd_sentiment_score']
                label = nh['sentiment_label']
                summary = nh['sentiment_summary'] or ''
                narrative_summary = f"Latest X narrative for {token_hint} ({nh['queried_at'].strftime('%b %d')}): {label} ({score}/10). {summary[:200]}"

        await conn.close()

        # 5. If data is thin and it's a price/movement question — fire live Grok price driver
        live_driver = ""
        price_keywords = {"down","up","drop","surge","pump","dump","crash","rally","move","moving","why","price"}
        is_price_question = any(w.lower() in price_keywords for w in q.split())
        if token_hint and is_price_question and len(all_rows) < 4:
            try:
                node = SentimentNode()
                # Estimate price change direction from question wording
                pct = -3.0 if any(w.lower() in {"down","drop","dump","crash","fell","falling"} for w in q.split()) else 3.0
                driver_result = await node.get_price_driver(token_hint, pct)
                explanation = driver_result.get("leading_explanation", "")
                factors = driver_result.get("key_factors", [])
                if explanation:
                    live_driver = f"LIVE GROK PRICE DRIVER for {token_hint}: {explanation}"
                    if factors:
                        live_driver += f" Key factors: {'; '.join(str(f) for f in factors[:3])}"
            except Exception as grok_err:
                print(f"[Collective] Grok price driver fallback failed: {grok_err}")

        # 6. Build context for Claude synthesis
        corpus_context = ""
        if all_rows:
            corpus_context = "RELEVANT CORPUS ENTRIES:\n"
            for r in all_rows[:6]:
                score_str = f" | Team avg: {r['avg_score']:.1f}/5" if r['avg_score'] else " | Unjudged"
                corpus_context += f"- [{r['domain'] or 'general'}] {r['description'][:180]}{score_str}\n"

        # 7. Synthesize with Claude
        anthropic_client = anthropic.Anthropic(api_key=os.getenv("ANTHROPIC_API_KEY"))

        system_prompt = """You are The Collective, Arca's internal crypto intelligence system.
Answer questions about crypto markets, tokens, and events using the provided internal data.
Be direct and opinionated. Lead with the most important insight.
Keep your answer to 3-5 sentences maximum. No bullet points.
If live Grok data is provided, prioritize it for price movement questions.
Reference team conviction data when available.
End with one actionable implication for a professional digital assets investor.
Never say you don't have data if live Grok data was provided."""

        user_content = f"""Question: {q}

{corpus_context if corpus_context else "No relevant corpus entries found."}
{f"TEAM CONVICTION: {conviction_summary}" if conviction_summary else ""}
{f"CROWD NARRATIVE: {narrative_summary}" if narrative_summary else ""}
{f"{live_driver}" if live_driver else ""}

Answer the question directly. Use live Grok data if available for price movement questions."""
